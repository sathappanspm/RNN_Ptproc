...
class RMTP_TIME(BaseRNNPtProc):
    ...
    def build_graph(self, learning_rate=1e-4):
        ...
        self._negllik = tf.multiply(-1.0, self.calc_loss(self._labels_flattened, self._rnnoutputs_flattened))
        self._cost = tf.reduce_sum(self._negllik)
    ...
    def calc_loss(self, current_time, rnnstates):
        ...
        return self._rate_t + tf.exp(self._hist_influence + self._bo) * (1/self._wo) - (1/self._wo)* tf.exp(self._rate_t)


class RMTP(BaseRNNPtProc):
    def __init__(self, hidden_size, batch_size, num_input_vars, **kwargs):
        if num_input_vars < 2:
    ...
    def init_variables(self):
        ...
        self._rnninputs = tf.concat([self._inputs[:, :, :1]] + list(self._inputemb.values()), axis=2)
    ...
    def build_graph(self, learning_rate=1e-4, loss_tradeoff=1):
        ...
        self._negllik = tf.multiply(-1.0, self.calc_time_loss(self._time_labels_flattened, self._rnnoutputs_flattened))
        self._timeloss = tf.reduce_sum(self._negllik)
        ...
        self._cost = tf.add_n([mloss for mloss in self._loss.values()]) + loss_tradeoff * self._timeloss
